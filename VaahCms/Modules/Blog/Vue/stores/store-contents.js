...
let model_namespace = 'VaahCms\\Modules\\Blog\\Models\\Content';


let base_url = document.getElementsByTagName('base')[0].getAttribute("href");
let ajax_url = base_url + "/blog/contents";
    ...
    actions: {
        ...
        async getItemAfter(data, res)
        {
            if(data)
            {
                ...
                this.$router.push({name: 'contents.index',query:this.query});
            }
        ...
        async formActionAfter (data)
        {
            switch (this.form.action)
            {
                ...
                case 'create-and-clone':
                    this.item.id = null;
                    this.$router.push({name: 'contents.form',query:this.query,params: { id: null }});
                    await this.getFormMenu();
                    break;
                case 'trash':
                case 'restore':
        ...
        toForm()
        {
            this.item = vaah().clone(this.assets.empty_item);
            this.getFormMenu();
            this.$router.push({name: 'contents.form',query:this.query})
        },
        //---------------------------------------------------------------------
        toView(item)
        {
            ...
            this.$router.push({name: 'contents.view', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        toEdit(item)
        {
            ...
            this.$router.push({name: 'contents.form', params:{id:item.id},query:this.query})
        },
        //---------------------------------------------------------------------
        isViewLarge()
        {
